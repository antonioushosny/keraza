<x-filament-panels::page>
    ...
    <div class="fi-section rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 p-5">
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-base font-black text-gray-950 dark:text-white">تقرير تصنيفات الامتحانات</h2>
                    @if($seasonName)
    ...
    @if(!$selectedClassId)
        ...
        <div class="fi-section rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 p-12 text-center mt-6">
            ...
            <h3 class="text-xl font-bold text-gray-950 dark:text-white">اختر فصل لعرض تقرير تصنيفات الامتحانات</h3>
            <p class="text-gray-500 dark:text-gray-400 mt-2 text-sm">حدد الفصل من القائمة أعلاه لرؤية درجات وتصنيفات الامتحانات التفصيلية لكل مخدوم.</p>
        </div>
    ...
    @else
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
            ...
            <div class="fi-section rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 p-4 flex items-center gap-4">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center text-xl bg-blue-50 dark:bg-blue-950/30 text-blue-500">🏷️</div>
                <div>
                    <div class="text-xs text-gray-500 dark:text-gray-400">تصنيفات الامتحانات النشطة</div>
                    <div class="text-lg font-black text-gray-950 dark:text-white">{{ count($categories) }}</div>
                </div>
            </div>
        </div>
                    ...
                    <thead>
                        <tr class="bg-gray-50 dark:bg-white/5 border-b border-gray-100 dark:border-white/10">
                            ...
                            @foreach($categories as $category)
                                <th wire:click="sortBy('category_{{ $category['id'] }}')" class="px-4 py-4 text-xs font-black text-gray-500 dark:text-gray-400 text-center min-w-[140px] cursor-pointer hover:bg-gray-100 dark:hover:bg-white/10 transition-colors">
                                    <div class="flex items-center gap-1 justify-center">
                                        <span>نسبة {{ $category['name'] }}</span>
                                        @if($sortField === 'category_' . $category['id'])
                                            <span class="text-[10px]">{{ $sortDirection === 'asc' ? '▲' : '▼' }}</span>
                                        @endif
                                    </div>
                                </th>
                            ...
                            <th wire:click="sortBy('overall')" class="px-4 py-4 text-xs font-black text-gray-500 dark:text-gray-400 text-center min-w-[120px] cursor-pointer hover:bg-gray-100 dark:hover:bg-white/10 transition-colors">
                                <div class="flex items-center gap-1 justify-center">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        @foreach($reportData as $index => $row)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/5 transition">
                                ...
                                @foreach($categories as $category)
                                    ...
                                    <td class="px-4 py-4 text-center">
                                        @if($pct !== null)
                                            ...
                                            @endphp
                                            <div class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold ring-1 {{ $badgeClass }}">
                                                <span>{{ $pct }}%</span>
                                            </div>
                                        @else
                                ...
                                <td class="px-4 py-4 text-center">
                                    @php
                                        $overallPct = $row['overall_percentage'];
