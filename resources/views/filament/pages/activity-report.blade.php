<x-filament-panels::page>
    ...
    <div class="fi-section rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 p-5">
        <div class="flex flex-col md:flex-row items-start md:items-center gap-4">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style="background: linear-gradient(135deg, #f97316, #ea580c);">🎨</div>
                <div>
                    <h2 class="text-base font-black text-gray-950 dark:text-white">تقرير الأنشطة المطور</h2>
                    @if($seasonName)
            ...
            <div class="w-full md:w-auto md:mr-auto flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                ...
                <select wire:model.live="selectedClassId"
                        class="w-full sm:w-48 text-sm font-bold rounded-lg px-4 py-2.5 transition
                                ...
                                text-gray-950 dark:text-white
                                focus:ring-2 focus:ring-orange-500 focus:border-orange-500">
                    <option value="">كل الفصول (اختياري)</option>
                    @foreach($classes as $class)
                        <option value="{{ $class['id'] }}">{{ $class['name'] }}</option>
                    @endforeach
                </select>

                @if($selectedActivityId && !empty($reportData))
                    <button wire:click="export" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-bold text-white bg-orange-600 border border-transparent rounded-lg hover:bg-orange-500 transition dark:bg-orange-500 dark:hover:bg-orange-400 focus:outline-none focus:ring-2 focus:ring-orange-500">
    ...
    @if(!$selectedActivityId)
        ...
        <div class="fi-section rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 p-12 text-center mt-6">
            ...
            <h3 class="text-xl font-bold text-gray-950 dark:text-white">اختر نشاطاً لعرض تقرير الأنشطة</h3>
            <p class="text-gray-500 dark:text-gray-400 mt-2 text-sm">حدد النشاط والفصل من القوائم أعلاه لرؤية درجات وتفاصيل أداء المخدومين.</p>
        </div>
    @elseif(empty($reportData))
        <div class="fi-section rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 p-12 text-center mt-6">
            <div class="text-6xl mb-4">📋</div>
            <h3 class="text-xl font-bold text-gray-950 dark:text-white">لا يوجد مشاركين في هذا النشاط</h3>
            <p class="text-gray-500 dark:text-gray-400 mt-2 text-sm">لم يتم تسجيل أي مخدوم في هذا النشاط بعد للموسم الحالي.</p>
        </div>
    @else
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
            <div class="fi-section rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 p-4 flex items-center gap-4">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center text-xl bg-orange-50 dark:bg-orange-950/30 text-orange-500">👥</div>
                <div>
                    <div class="text-xs text-gray-500 dark:text-gray-400">إجمالي المشاركين بالنشاط</div>
            ...
            <div class="fi-section rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 p-4 flex items-center gap-4">
                ...
                <div>
                    <div class="text-xs text-gray-500 dark:text-gray-400">متوسط التقييم الإجمالي</div>
                    <div class="text-lg font-black text-gray-950 dark:text-white">
                        {{ round(collect($reportData)->avg('final_score'), 1) }}%
                    </div>
                </div>
            </div>
        </div>
                ...
                <table class="w-full text-right border-collapse">
                    <thead>
                        <tr class="bg-gray-50 dark:bg-white/5 border-b border-gray-100 dark:border-white/10">
                            ...
                            <th class="px-4 py-4 text-xs font-black text-gray-500 dark:text-gray-400 text-center">الفصل</th>
                            <th class="px-4 py-4 text-xs font-black text-gray-500 dark:text-gray-400 text-center">حضور الأنشطة</th>
                            <th class="px-4 py-4 text-xs font-black text-gray-500 dark:text-gray-400 text-center">مهام الأنشطة</th>
                            <th class="px-4 py-4 text-xs font-black text-gray-500 dark:text-gray-400 text-center">التقييم العام للنشاط</th>
                            <th class="px-4 py-4 text-xs font-black text-gray-500 dark:text-gray-400 text-center min-w-[120px]">الدرجة الكلية الموزونة</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        @foreach($reportData as $index => $row)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/5 transition">
                                ...
                                <td class="px-4 py-4 text-center">
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-400 ring-1 ring-blue-600/20">
                                        {{ $row['attendance_score'] }}%
                                    </span>
                                </td>
                                <td class="px-4 py-4 text-center">
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold bg-pink-50 dark:bg-pink-950/30 text-pink-700 dark:text-pink-400 ring-1 ring-pink-600/20">
                                        {{ $row['tasks_score'] }}%
                                    </span>
                                </td>
                                <td class="px-4 py-4 text-center">
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold bg-amber-50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-400 ring-1 ring-amber-600/20">
                                        {{ $row['evaluation_score'] }}%
                                    </span>
                                </td>
                                <td class="px-4 py-4 text-center font-black text-sm text-orange-600 dark:text-orange-400">
                                    {{ $row['final_score'] }}%
                                </td>
                            </tr>
                        @endforeach
